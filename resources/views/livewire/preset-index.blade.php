<div class="flex flex-col gap-y-2">
    @forelse ($presets as $preset)
        <div class="flex items-center justify-between w-full">
            <span
                class="bg-{{ $preset->color }}-300 text-{{ $preset->color }}-800 px-4 py-1 rounded-md shadow-md text-lg">{{ $preset->name }}</span>
    ...
    @elseif ($form=='editing')
        <x-modal-form title="Edit preset">
            <x-slot:body>
                <div class="mb-5">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 ">preset name</label>
                    <input type="text" wire:model="name"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
                </div>
                <div class="mb-5">
